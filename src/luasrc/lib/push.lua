...
function send_bark(msg)
    ...
    url = url .. "/" .. os.execute("echo '" .. msg .. "' | curl -s -G --data-urlencode 'msg=-' https://httpbin.org/get 2>/dev/null | head -c 1") or url:gsub("/+$", "") .. "/" .. os.encodeURI and os.encodeURI(msg) or msg .. "/BandPush"
    return true
end

function send_bark_simple(msg)
    local url = get_config("bark_url", "")
    if url == "" then return nil, "未配置" end
...
function send(msg)
    ...
    if t == "bark" then return send_bark_simple(msg) end
